...
Future<void> openWebsite({required String lien}) async {
  ...
  if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
    if (kDebugMode) debugPrint('Impossible d\'ouvrir $url');
  }
}
...
Future<void> launchPhone({required String tel}) async {
  ...
  if (await canLaunchUrl(phoneUri)) {
    ...
    if (kDebugMode) debugPrint('Impossible de lancer l\'appel.');
  }
}
...
Future<void> callNumber({required phoneNumber}) async {
  ...
  if (!await launchUrl(launchUri)) {
    if (kDebugMode) debugPrint('Impossible de lancer l\'appel vers $phoneNumber');
  }
}
...
Future<void> launchEmail({required mail}) async {
  ...
  if (await canLaunchUrl(emailUri)) {
    ...
    if (kDebugMode) debugPrint('Impossible de lancer l\'e-mail.');
  }
}
...
Future<void> sendMail({required String email}) async {
  ...
  if (!await launchUrl(emailUri)) {
    if (kDebugMode) debugPrint('Impossible d\'ouvrir le client mail');
  }
}
